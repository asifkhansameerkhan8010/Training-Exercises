...
export class BackupService {
  ...
  private storeBackupHistory() {
    ...
    const newBackup = {
      ...
      by:loggedInUser || "Unknown User",
      description: 'Backup created' 
    };
    backupHistory.push(newBackup);
    localStorage.setItem('backupHistory', JSON.stringify(backupHistory));  // Store updated history
  }

  private getBackupHistory() {
    const history = localStorage.getItem('backupHistory');
    return history ? JSON.parse(history) : [];  // Retrieve backup history from local storage
  }
}
